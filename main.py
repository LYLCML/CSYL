...
def training_main():
    ...
    for epoch in range(mth.epoch, tot_epoch):
        ...
        if epoch >= 180:
            ...
            scores, thresh, pred, max_auroc, max_aurocZ, auroc_kn, auroc_unk = mth.knownpred_unknwonscore_test(
                test_loader)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--ds', type=str, required=False, default="None",
    ...
    parser.add_argument('--method', type=str, required=False, default="ours",
    ...
    saving_path = saving_path + "alpha_" + str(config['category_model']['lambda1']) + "_beta_" + str(
        config['category_model']['lambda2']) + "_safe_lr_" + str(config['category_model']['safe_lr']) + "_"
    ...
    # mth.load_model("./save//cifar10/e_m5/" + 'alpha_0.0001_beta_1_safe_lr_0_model.pth')
    history = []
    evaluation = metrics.OSREvaluation(test_loader)
    # mth.knownpred_unknwonscore_test(test_loader)
    ...
    if not args.test:
        ...
        save_everything()
        overall_testing()
        print("Overall: LastAcc", last_acc, " LastAuroc", last_auroc, " BestAcc", best_acc, " BestAuroc", best_auroc,
              "CWAuroc", cwauc)
    else:
        load_everything()
        overall_testing()
