...
LARGE_OOD_PATH = '../deep_data/LARGE_OOD'
IMAGENET_PATH = '../deep_data/imagenet-1k'
...
def get_combined_dataloaders(args,settings):
    ...
    test1 = get_combined_dataset('test', settings['train'])
    test2 = get_combined_dataset('test',settings['test'])
    return torch.utils.data.DataLoader(train_labeled, batch_size=args.bs, shuffle=istrain_mode, num_workers=workers,pin_memory=True,drop_last = use_droplast) if train_labeled is not None else None, \
            torch.utils.data.DataLoader(test1, batch_size=args.bs, shuffle=False, num_workers=test_workers,pin_memory=args.gpu != 'cpu') if test1 is not None else None, \
            torch.utils.data.DataLoader(test2, batch_size=args.bs, shuffle=False, num_workers=test_workers,pin_memory=args.gpu != 'cpu') if test2 is not None else None
...
def load_partitioned_dataset(args,ds):
    ...
    a,b,c= get_combined_dataloaders(args,settings)
    return a,b,c,ds_classnum_dict[settings['test'][0]["dataset"]]
